            ...
            scope.runReport = function () {
                ...
                if (scope.errorDetails.length == 0) {
                    ...
                    scope.pdfData = function() {
                        scope.formData = {};     
                        scope.formData.outputType = 'csv';
                        var outputType = scope.formData.outputType;
                          /* window.open($rootScope.hostUrl + API_VERSION + '/runreports/' + scope.reportName+
                                          '?output-type=PDF&tenantIdentifier=' + $rootScope.tenantIdentifier + '&locale=' + scope.optlang.code + '&dateFormat=' + scope.df+ '&exportPDF=true');
*/
                        window.open($rootScope.hostUrl + API_VERSION + "/runreports/" + scope.reportName+
       '?output-type=PDF' + scope.formData.outputType + "&tenantIdentifier=" + $rootScope.tenantIdentifier + "&locale=" + scope.optlang.code + "&dateFormat=" + scope.df + "&R_officeId=" + scope.formData.R_officeId + "&R_startDate=" + scope.formData.R_startDate + "&R_endDate=" + scope.formData.R_endDate + '&exportPDF=true');
