(function(module) {
	mifosX.controllers = _.extend(module, {
		AssignedTicketController: function(scope,webStorage, routeParams, location,$uibModal, resourceFactory, paginatorService,PermissionService,$rootScope,dateFilter,route) {
			...
			scope.routeToticket = function(ticket){
				if(ticket.type =='Customer'){
		        	if(PermissionService.showMenu('READ_CLIENT'))
		        		location.path('/viewTicket/'+ticket.clientId+'/'+ticket.id);
				} else if(ticket.type =='Office'){
					if(PermissionService.showMenu('READ_OFFICE'))
		        		location.path('/viewOfficeTicket/'+ticket.clientId+'/'+ticket.id);
				}
	        };
	     ...
	     /*   scope.openTickets = paginatorService.paginate(scope.openTicketFetchFunction, 14);*/
	        ...
	        scope.getTicketHistory=function(offset, limit, callback){
	        	resourceFactory.getAllTicketResource.getAllDetails({offset: offset, limit: limit, fromDate : scope.fromDate, toDate :scope.toDate, statusType :scope.statusType,type :scope.type}, callback);

			};
	        
	        scope.getTrans=function(fromDate,toDate,statusTypes,ticketsType){
