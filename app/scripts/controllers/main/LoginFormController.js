        ...
        LoginFormController: function (scope, authenticationService, resourceFactory, httpService, $timeout) {
            ...
            scope.$on("UserAuthenticationFailureEvent", function (event, data, status) {
                ...
                if(status != 401) {
                    ...
                    scope.load = false;
                } else if(scope.loginCredentials.username == undefined && scope.loginCredentials.password == undefined){
                	scope.authenticationErrorMessage = 'error.login.failed';
                    scope.load = false;
                } else if(scope.loginCredentials.username == undefined) {
                    scope.authenticationErrorMessage = 'error.login.username';
                    scope.load = false;
                 } else if(scope.loginCredentials.password == undefined) {
                   scope.authenticationErrorMessage = 'error.login.password';
                   scope.load = false;
                } else {
                	scope.authenticationErrorMessage = 'error.login.invaild';
                    scope.load = false;
                }
                delete scope.loginCredentials.password;
            });
