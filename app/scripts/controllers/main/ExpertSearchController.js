(function (module) {
    mifosX.controllers = _.extend(module, {
        ExpertSearchController: function (scope, resourceFactory, location,localStorageService,routeParams,webStorage, $rootScope) {
            ...
            resourceFactory.allDeviceResource.get({officeId: routeParams.id} , function(data) {
             ...
             resourceFactory.userListResource.get({userId: scope.userDetails.userId}, function (data) {
                 ...
                 $rootScope.officeData = {
                  ...
                  webStorage.add("officeData",$rootScope.officeData);
                  if(scope.user.officeType == "LCO"){
        	        	location.path('/viewoffice/' + scope.user.officeId);
        	        }
             });
           ...
           //count duplicates for clients
			...
			scope.refresh = function() {
	        	resourceFactory.refreshDashboardResource.get();
				resourceFactory.allDeviceResource.get({officeId: routeParams.id} , function(data) {
					scope.deviceData = data;   
			  });
			}
    ...
    mifosX.ng.application.controller('ExpertSearchController', ['$scope', 'ResourceFactory', '$location','localStorageService','$routeParams','webStorage', '$rootScope', mifosX.controllers.ExpertSearchController]).run(function ($log) {
