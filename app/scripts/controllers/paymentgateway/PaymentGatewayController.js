(function(module) {
	mifosX.controllers = _.extend(module,{
			PaymentGatewayController : function(scope, webStorage, route, $uibModal, routeParams, location, resourceFactory, 
					paginatorService, PermissionService, dateFilter, API_VERSION, $rootScope , $http) {
					...
					*/
					...
					// for rave pay reprocess
					scope.ravepayReprocess = function() {								
						scope.errorStatus = [];								
						scope.errorDetails = [];																
						$uibModal.open({											
							templateUrl : 'ravepayreprocess.html',											
							controller : ravepayreprocessController,											
							resolve : {}										
						});							
					};

					var ravepayreprocessController = function($scope, $uibModalInstance) {
						$scope.transactionId = null;
						$scope.flag = false;

						$scope.accept = function(transactionId) {				
							var data = {
								txref: transactionId,
								flwref: "INVIEWTECH/TR43533588318",
							}
							var url = $rootScope.hostUrl + API_VERSION + '/ravepay/orderlock?txref=' + data.txref + '&flwref=' + data.flwref;
							$http.post(url).then(function (response){
								console.log(response);
									$scope.msg = "Transaction is being reprocessed!";
									alert($scope.msg);
							}, function (err) {
								console.log("errorTest",err);
									$scope.msg = "Transaction is already Success!"
									alert($scope.msg);
							});
							$uibModalInstance.close('delete');							
						};
