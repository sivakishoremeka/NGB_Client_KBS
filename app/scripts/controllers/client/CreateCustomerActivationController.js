(function(module) {
  mifosX.controllers = _.extend(module, {
	  CreateCustomerActivationController: function(scope,webStorage,routeParams, resourceFactory, location, http,filter,PermissionService, dateFilter,$rootScope,API_VERSION) {
	      ...
	      scope.isGlobalConfigService = false;
	      ...
	      scope.globalConfigs =  webStorage.get("global_configuration");
	        for(var i in scope.globalConfigs){
	        	if(scope.globalConfigs[i].name == "isService"){
	        		scope.isGlobalConfigService = scope.globalConfigs[i].enabled;
	        		console.log(scope.isGlobalConfigService);
	        		if(scope.isGlobalConfigService == true){
		        		var serviceValue = scope.globalConfigs[i].value;
		        		var servivceData = JSON.parse(serviceValue);
		        		scope.formData.serviceCode = servivceData.serviceType;
	      ...
	      /*END*/
	       ...
	       /* This is used to get Service Param details based on service code*/
	        if (scope.formData.serviceCode != null){
	        	resourceFactory.getServiceDetailsResource.get({serviceCode: scope.formData.serviceCode}, function(data) {
					  scope.serviceDetails = angular.copy(data);
					  scope.formData.serviceId = scope.serviceDetails.id;
				  });
	        }
	      /*END*/
		  ...
		  /*This is used to get inventory data based global config isService*/
	        if(scope.isGlobalConfigService == true){
	        	scope.getOneTimeSale();	
	        }
	     /*END*/
        ...
        scope.getItemData = function(item,model,label){
	        	...
	        	if(item || model || label){
		        	...
		        	resourceFactory.itemMasterDetailTemplateResource.get({query : serialNum,clientId:scope.clientId,officeId:scope.formData.officeId},function(data) {
		        	   if(data){
		        		   ...
		        		   scope.formData.networkId = data.networkId;
		        		   scope.formData.systemName = data.systemName;
		        		   if("Y" == data.isPairing){
		      ...
		      //user cataloge resource to select plans based on sales plan category
		       ...
		       scope.submit = function(){
		    	   ...
		    	   if(scope.isGlobalConfigService == true) {
		    		   scope.customerData.provisioningSystem =  scope.formData.systemName;
		    	   } else {
