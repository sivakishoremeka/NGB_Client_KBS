        ...
        ViewClientController: function (scope, routeParams, route, location, resourceFactory, http, $uibModal, API_VERSION, $rootScope, 
			...
			var getDetails = function() {
				...
				resourceFactory.client360resource.get({key:routeParams.key,value: routeParams.id}, function (data) {     
					...
					/* if serial no exists STB and SC values will be changed from null to actual values */
					scope.stbSerialNo = null;
					scope.scSerialNo = null;
					if(data.oneTimeSaleData.length != 0){
						if(data.oneTimeSaleData[0].serialNo != null){
							scope.stbSerialNo = data.oneTimeSaleData[0].serialNo;
						}
						if(data.oneTimeSaleData.length == 2){
                            if(data.oneTimeSaleData[1].serialNo != null){
                                  scope.scSerialNo = data.oneTimeSaleData[1].serialNo;
                             }
                       }
					}
					...
					$rootScope.ClientData = {
						...
						stbSerialNo : scope.stbSerialNo,
						scSerialNo : scope.scSerialNo,
